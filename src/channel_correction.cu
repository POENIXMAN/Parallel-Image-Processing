...
__global__ void PictureKernel(png_byte *d_P, int height, int width)
{
    ...
    if ((Row < height) && (Col < width))
    {
        ...
        float R_new = 1.0 * r + 0.0 * g + 0.0 * b;
        float G_new = 0.0 * r + 1.2 * g + 0.0 * b;
        float B_new = 0.0 * r + 0.0 * g + 1.0 * b;

        R_new = fminf(fmaxf(R_new, 0.0f), 1.0f);
        G_new = fminf(fmaxf(G_new, 0.0f), 1.0f);
        B_new = fminf(fmaxf(B_new, 0.0f), 1.0f);

        d_P[(Row * width + Col) * 3 + 0] = static_cast<png_byte>(R_new * 255.0f);
        d_P[(Row * width + Col) * 3 + 1] = static_cast<png_byte>(G_new * 255.0f);
        d_P[(Row * width + Col) * 3 + 2] = static_cast<png_byte>(B_new * 255.0f);
    }
}
...
void process_on_host(PNG_RAW *png_raw)
{
    ...
    for (int i = 0; i < png_raw->width * png_raw->height; i++)
    {
        ...
        float r_new = 1.0f * r + 0.0f * g + 0.0f * b;
        float g_new = 0.0f * r + 1.2f * g + 0.0f * b;
        float b_new = 0.0f * r + 0.0f * g + 1.0f * b;

        png_raw->buf[i * 3] = static_cast<png_byte>(fminf(fmaxf(r_new * 255.0f, 0.0f), 255.0f));
        png_raw->buf[i * 3 + 1] = static_cast<png_byte>(fminf(fmaxf(g_new * 255.0f, 0.0f), 255.0f));
        png_raw->buf[i * 3 + 2] = static_cast<png_byte>(fminf(fmaxf(b_new * 255.0f, 0.0f), 255.0f));
    }
